...
export default class ConnectionRequest extends React.Component{
  ...
  fetchUserIDs(){
    ...
    .catch(error => console.log('error', error));
    fetch("https://webdev.cse.buffalo.edu/hci/api/api/commitment/connections/", requestOptionsGet)
    .then(response => response.json())
    .then(result => {
      for(var i = 0; i < result[0].length; i++) {
        let name = "";
        let connectionId, id = -1;
        if(result[0][i].attributes.status == null){
          continue
        }
        if(result[0][i].attributes.status == "accepted" && result[0][i].fromUserID == this.props.userid ){
          name = result[0][i].toUser.attributes.firstName + " " + result[0][i].toUser.attributes.lastName[0] + "."
          console.log("fromuser",result[0][i].toUser.attributes.firstName + " " + result[0][i].toUser.attributes.lastName[0] + ".");
          console.log(name);
        ...
        else if(result[0][i].attributes.status == "accepted" && result[0][i].toUserID == this.props.userid ){
          name = result[0][i].fromUser.attributes.firstName + " " + result[0][i].fromUser.attributes.lastName[0] + "."
          console.log("touser",result[0][i].fromUser.attributes.firstName + " " + result[0][i].fromUser.attributes.lastName[0] + ".");
          console.log(name);
    ...
    console.log(user_id_map);
      return (
          <div className="App">
                  ...
                  {this.state.names.map((name) => {
                    return(<div key={name}>
                      <div className={connections.topdiv}>
                        <img className={connections.picturecircle} src={"https://webdev.cse.buffalo.edu/hci/api/uploads/files/DOo1Ebbt8dYT4-plb6G6NP5jIc9_l_gNlaYwPW4SaBM.png"} alt="default img"/> 
                        <Link to={"/profile/"+user_id_map[name]} className={connections.name} >{name}</Link>
                        <div className={connections.stars}>
                          <img className={connections.star1} src={starIcon} alt="star"/>
                          <img className={connections.star2} src={starIcon} alt="star"/>
                          <img className={connections.star3} src={starIcon} alt="star"/>
                          <img className={connections.star4} src={starIcon} alt="star"/>
                          <img className={connections.star5} src={starIcon} alt="star"/>
                        </div>
                  ...
                  <img className={connections.picturecircle} src={"https://webdev.cse.buffalo.edu/hci/api/uploads/files/x9sRVcdg8C5vip7apFO5cuR1jus4w00_6oOtB2bFKqg.png"} alt="img of Lisa Moan" hidden />
          </div>
      );
    }
}